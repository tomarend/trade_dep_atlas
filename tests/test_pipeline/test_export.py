...
_COMPOSITE_SCHEMA = {
    ...
    "substitutability_score": pl.Float64,
...
def _make_composite_df(n: int = 1000) -> pl.DataFrame:
    ...
    for i in range(n):
        ...
        sub = round(random.uniform(0.1, 1.0), 4)
        gexhhi = round(sub ** 2, 6)
        composite = round(0.35 * hhi + 0.35 * geo + 0.30 * sub, 6)
        rows.append({
            "importer_iso3": imp,
            "concorded_hs6": hs6,
            "year": 2022,
            ...
            "global_export_hhi": gexhhi,
            "substitutability_score": sub,
            "flags": [],
            "hs22_only": False,
...
def _write_flags_parquet(tmp_path: Path) -> Path:
    flags_path = tmp_path / "flags_scores.parquet"
    rows = [
        {"hs6": hs6, "flags": [], "global_export_hhi": 0.3,
         "crm_listed_since": None, "hs22_only": False}
        for hs6 in _HS6_CODES
    ]
    pl.DataFrame(rows, schema=_FLAGS_SCHEMA).write_parquet(flags_path)
    return flags_path
...
def test_duckdb_creates_fact_table(tmp_path):
    ...
    flags_path = _write_flags_parquet(tmp_path)
    db_path = tmp_path / "test.duckdb"

    build_duckdb(db_path, composite_dir, tmp_path / "georisk.parquet", flags_path, tmp_path)
...
def test_duckdb_expected_columns(tmp_path):
    """dependency_scores has all expected v2 columns including substitutability_score."""
    composite_dir = _write_composite_parquet(tmp_path)
    flags_path = _write_flags_parquet(tmp_path)
    db_path = tmp_path / "test.duckdb"
    ...
    expected = {
        ...
        "global_export_hhi", "flags", "hs22_only",
    }
